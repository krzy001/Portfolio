...
void CPlane::Create(string directory, string filename, float width, float height, float textureRepeat)
{
	...
	// Load the texture
	m_texture0.Load(directory+filename, true);
	m_texture1.Load(directory+"dirtpile01.jpg", true);

	m_directory = directory;
	m_filename = filename;

	// Set parameters for texturing using sampler object
	m_texture0.SetSamplerObjectParameter(GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	m_texture0.SetSamplerObjectParameter(GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	m_texture0.SetSamplerObjectParameter(GL_TEXTURE_WRAP_S, GL_REPEAT);
	m_texture0.SetSamplerObjectParameter(GL_TEXTURE_WRAP_T, GL_REPEAT);
